...
- 自研回归：`.venv/bin/python tests/test_verifier.py`（27）、`tests/run_suite.py`（45）、`tests/test_baseline.py`（22，独立临时库、写操作不污染线上 data/app.db）、`tests/test_golden.py`（16）、`tests/test_context.py`（13）、`tests/test_entities.py`（18）、`tests/test_store.py`（20）、`tests/test_retrieval.py`（16）、`tests/test_oauth.py`（28）、`tests/test_audit.py`（12）、`tests/test_public_app.py`（10）、`tests/test_web_page.py`（内嵌 JS 语法 3）、`tests/test_import.py`（导入适配器 10）；全量一键 `./tools/run_all_tests.sh`（含 guard 双侧）
...
- 上线补完资产：`tools/import_customer_data.py`（CSV→app.db/documents.json 导入适配器，`--dry-run` 演练、`--wipe` 换真实数据先清空合成数据；步骤见 `docs/数据接入.md`）、`tools/backup.sh`（`data/` 一致性快照 + `BACKUP_KEEP` 轮转 + 0600，恢复见 `docs/备份与恢复.md`）、`deploy/`（systemd 单元 + `connector.env.example` + 无 systemd 的 `watchdog.sh`）、`tools/run_all_tests.sh` + `.github/workflows/tests.yml`（CI，`PY=python` 覆盖解释器）
